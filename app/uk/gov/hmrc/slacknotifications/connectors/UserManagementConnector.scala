...
class UserManagementConnector @Inject()(
  ...
  def getLdapUser(ldapUsername: String)(using HeaderCarrier): Future[Option[User]] =
    httpClientV2
      .get(url"$baseUrl/user-management/users/$ldapUsername")
      .execute[Option[User]]

  def getGithubUser(githubUsername: String)(using HeaderCarrier): Future[List[User]] =
    httpClientV2
      .get(url"$baseUrl/user-management/users?github=$githubUsername")
      .execute[List[User]]

  def getTeamUsers(team: String)(using HeaderCarrier): Future[Seq[User]] =
    httpClientV2
      .get(url"$baseUrl/user-management/users?team=$team")
      .execute[Seq[User]]

  def getTeamSlackDetails(teamName: String)(using HeaderCarrier): Future[Option[TeamDetails]] =
    httpClientV2
